"""Latent autoregressive training entrypoint."""
...
from src.models.latent_autoregressive.config import ARConfig, build_config
from src.models.latent_autoregressive.data import (
    LatentSequenceDataset,
    ensure_preprocessed,
)
from src.models.latent_autoregressive.model import (
    LatentAR,
    load_latent_autoregressive,
)
...
class LatentARTrainerSequential(Trainer):
    ...
    def __init__(
        ...
        """Initialize latent autoregressive trainer."""
        self.ae = autoencoder
    ...
    def _profile_epoch(self, warmup_batches: int = 3, prof_batches: int = 1) -> None:
        ...
        if not torch.distributed.is_available() or (
            ...
            trace_path = self._profile_trace_path("latent_autoregressive")
...
def train(dataset_config, force_preprocess: bool = False) -> None:
    """Train latent autoregressive model with cached sequence artifacts."""
    ...
    training_dataset_wrapped = LatentSequenceDataset(
        dataset_config,
        training_dataset,
        training_indices,
        ae_config.latent_dim,
    )
    validation_dataset_wrapped = LatentSequenceDataset(
        dataset_config,
        validation_dataset,
        validation_indices,
        ae_config.latent_dim,
    ...
    training_dataloader = tensor_to_dataloader(ar_config, training_dataset_wrapped)
    validation_dataloader = tensor_to_dataloader(
        ar_config,
        validation_dataset_wrapped,
    )
    latent_ar = load_latent_autoregressive(LatentAR, dataset_config, ar_config)
    optimizer, scheduler = load_objects(latent_ar, ar_config)
    trainer = LatentARTrainerSequential(
        dataset_config,
        ae_config,
        ar_config,
        loss_functions,
