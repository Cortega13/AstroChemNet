"""Configuration dataclass for latent autoregressive model."""
...
@dataclass
class ARConfig:
    """Latent autoregressive configuration with hyperparameters and paths."""
    ...
    def __post_init__(self) -> None:
        ...
        self.pretrained_model_path = self.dataset_config.model_path("latent_autoregressive", "model.pth")
        self.save_model_path = self.pretrained_model_path


def build_config(dataset_config: DatasetConfig, ae_config: AEConfig, **overrides) -> ARConfig:
    """Build latent autoregressive config for a dataset."""
    return ARConfig(dataset_config=dataset_config, ae_config=ae_config, **overrides)
