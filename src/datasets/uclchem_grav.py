...
def maybe_cap_rows(
    ...
    """Cap rows while preserving full model trajectories when possible."""
    if max_rows is None or len(data_array) <= max_rows:
        return data_array

    model_ids = data_array[:, model_col_idx].astype(np.int64)
    change_indices = np.flatnonzero(np.diff(model_ids) != 0) + 1
    complete_model_ends = change_indices[change_indices <= max_rows]

    if complete_model_ends.size == 0:
        capped = data_array[:max_rows]
        print(
            f"Row cap {max_rows} falls inside the first model trajectory; using the first {len(capped)} rows."
        )
        return capped

    capped_end = int(complete_model_ends[-1])
    capped = data_array[:capped_end]
    print(
        f"Applied row cap near {max_rows}; using {len(capped)} rows to keep full model trajectories."
    )
    return capped
...
def preprocess_uclchem_grav(
    ...
    Args:
        ...
        max_rows: Optional cap on the number of source rows to preprocess
    """
    # Determine paths
    ...
    with open(os.path.join(project_root, columns_path)) as f:
    ...
    data_array = maybe_cap_rows(data_array, model_col_idx, max_rows)
